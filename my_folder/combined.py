...
def sub_vectors(v1: ldt.Vector, v2: ldt.Vector) -> list:
    return list(map(float.__sub__, v1, v2))
...
class Canvas:
    ...
    def render_hands(self, event):
        ...
        for i in range(0, len(event.hands)):
            ...
            # Check for pinching and display "Pinch" if detected
            thumb = location_end_of_finger(hand, 0)  # Thumb
            index = location_end_of_finger(hand, 1)  # Index finger
            if fingers_pinching(thumb, index):
                cv2.putText(self.output_image, "Pinch", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
