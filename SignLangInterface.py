...
from RealTimeDetection import RealTimePredictionListener
...
class Canvas:
    ...
    def update_predicted_word(self, word):
        self.predicted_word = word
    
    def prediction_handler(self, prediction):
        self.update_predicted_word(prediction)
            ...
            for index_digit in range(0, 5):
                ...
                for index_bone in range(0, 4):
                    ...
                    if self.hands_format == "Skeleton":
                        ...
                        #self.update_predicted_word('hi')  # Update the canvas with the predicted word

            
            cv2.putText(self.output_image, f"Predicted Word: {self.predicted_word}", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
def main():
    canvas = Canvas()
    prediction_listener = RealTimePredictionListener(prediction_callback=lambda prediction: canvas.prediction_handler(prediction))
    ...
    connection.add_listener(prediction_listener)
